...
def shift_and_resize_image(img, shift_x, shift_y, nw, nh):
    w, h = img.shape
    enlarged = imresize(img, nw, nh)
    return enlarged[shift_x:shift_x + w, shift_y:shift_y + h]

# shift_x, shift_y 없이 이미지 크기만 조정하는 함수
def imresize(img, nw, nh):
    # NumPy 배열을 Pillow 이미지로 변환
    img_pil = Image.fromarray(img)

    # 이미지 크기 조정 (Pillow의 resize 사용)
    enlarged_pil = img_pil.resize((nw, nh), Image.ANTIALIAS)

    # 다시 NumPy 배열로 변환
    enlarged = np.array(enlarged_pil)
    ...
    if type(resize_fix) == int:
        ...
        if verbose:
            ...
            print('resize_w:', resize_w, \
                  '[origin_w %d / origin_h %d]' % (origin_w, origin_h))
